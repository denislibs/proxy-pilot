...
fn parse(head: &[u8]) -> Result<Head, HeadError> {
    ...
    for line in lines {
        ...
        // Строки режутся по \r\n, а значения только тримятся — одиночный CR
        // или LF внутри значения переживает разбор и уходит в origin
        // дословно, то есть клиент дописывает заголовки в НАШ запрос.
        // Отвергнуть дешевле, чем чистить.
        if k.contains(['\r', '\n']) || v.contains(['\r', '\n']) {
            return Err(HeadError::Malformed);
        }
...
mod tests {
    ...
    async fn oversized_head_is_rejected() {
        ...
        big.extend(std::iter::repeat_n(b'x', 9000));
        let mut cursor = std::io::Cursor::new(big);
        assert!(matches!(
            read_head(&mut cursor, 4096).await,
    ...
    #[tokio::test]
    async fn header_value_with_a_bare_cr_or_lf_is_rejected() {
        // Строки режутся по \r\n, а значения только тримятся, поэтому
        // одиночный \n внутри значения переживает разбор и уходит в origin
        // дословно: клиент подмешал бы свои заголовки в наш запрос.
        assert!(matches!(
            head_of(b"GET http://h/ HTTP/1.1\r\nX: a\nInjected: yes\r\n\r\n").await,
            Err(HeadError::Malformed)
        ));
        assert!(matches!(
            head_of(b"GET http://h/ HTTP/1.1\r\nX: a\rInjected: yes\r\n\r\n").await,
        ...
        // имя заголовка уязвимо ровно так же
        assert!(matches!(
            head_of(b"GET http://h/ HTTP/1.1\r\nX\nInjected: yes\r\n\r\n").await,
            Err(HeadError::Malformed)
        ));
