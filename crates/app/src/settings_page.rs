...
/// (см. докблок `tunnel_log`). Имя адаптера, по которому раньше решала
/// `tunnel_state`, всё ещё используется, но только для `foreign_tunnel_up`
/// — и только пока `our_tunnel_up == false` (см. [`tunnel_section`]).
...
pub struct TunnelSnapshot {
    ...
    /// `winnet::tunnel_log::liveness(profile_name) == Up` — единственный
    /// источник «поднят ли НАШ туннель», ключуемый именем профиля, а не
    /// именем адаптера (см. докблок [`TUNNEL_PROFILE_NAME`]).
    pub our_tunnel_up: bool,
    /// Не удалось прочитать лог, чтобы решить `our_tunnel_up` честно —
    /// `Some` не гасит кнопки, а замещает их предупреждением: раздел,
    /// признающий «не знаю», лучше раздела, который молча блокирует обе
    /// кнопки (тот самый дедлок, который был найден в round 1).
    pub liveness_error: Option<String>,
    /// `tunnel_state::foreign_tunnel_up` по живой таблице маршрутов — уже
    /// не отвечает за «поднят ли наш», это единолично `our_tunnel_up`
    /// выше. Смысл у этого поля только пока `our_tunnel_up == false`:
    /// предупредить ДО подъёма, что какой-то туннельный адаптер уже несёт
    /// наши офисные подсети (свой ли забытый «хвост» или правда чужой VPN
    /// — различить по алиасу адаптера нельзя, см. [`TUNNEL_PROFILE_NAME`],
    /// поэтому раздел говорит «занято», не утверждая, чьё).
    pub foreign_tunnel_up: bool,
    /// Не удалось прочитать живую таблицу маршрутов — про `foreign_tunnel_up`
    /// сказать нечего честно. Гасит только кнопку подъёма (опускание своего
    /// туннеля от этой проверки не зависит вовсе).
...
async fn raise_tunnel(state: &SettingsState) -> Outcome {
    ...
    // подтверждённая логом поднятость делает повторный подъём избыточным,
    // но не опасным. Блокируют только те два случая, где раздел вовсе не
    // рисует кнопку подъёма: неизвестность таблицы маршрутов и обнаруженный
    // на ней туннель, несущий наши подсети, — оба имеют смысл только пока
    // `our_tunnel_up == false`.
    if snap.liveness_error.is_none() && !snap.our_tunnel_up {
