...
pub fn build_profile(source: &str, routes: &[Ipv4Net]) -> Result<String, ProfileError> {
    ...
    for (i, kind) in classified.iter().enumerate() {
        ...
        match kind {
            ...
            // невозможно. В релизе молчим, чтобы не ронять сборку профиля
            // из-за нашей же ошибки, но в тестах падаем громко: тихое
            // удаление строки с маркером — ровно тот отказ, который в этой
            // задаче возвращался трижды, и он обязан быть заметным сразу,
            // а не через три круга ревью.
            LineKind::Begin | LineKind::End => {
                debug_assert!(
                    false,
                    "drop_mask пропустил маркер: строка дошла до сборки вместо вычистки"
                );
            }
        }
    }
...
// Несимметрично к `inline_tag_open` по умыслу: непарный ОТКРЫВАЮЩИЙ тег —
// ошибка (`ProfileError`), непарный ЗАКРЫВАЮЩИЙ просто сохраняется как
// обычная строка. Оба одинаково нелюбимы OpenVPN, но отказываем мы там, где
// иначе испортили бы содержимое: незакрытый блок утаскивал бы за собой
// остаток файла. Закрывающий-сирота никого не утаскивает.
...
fn drop_mask(classified: &[LineKind]) -> Vec<bool> {
    ...
    for (i, kind) in classified.iter().enumerate() {
        match kind {
            ...
            // Вычищаем только на верхнем уровне. Внутри встроенного блока
            // содержимое непрозрачно: строка вида `setenv opt ...` внутри
            // <ca> это часть PEM, а не директива. Отличие от раундов 1-2,
            // где вычищалось где угодно; для <connection> OpenVPN setenv и
            // так не допускает, так что случай почти умозрительный.
            LineKind::TopLevel(line) => {
