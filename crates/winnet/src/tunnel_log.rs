...
//! остановки, и `liveness` останется выдавать `Up` дольше, чем это правда,
//! — та же категория допущения, что и «our_alias — предположение, не
//! гарантия» была у предыдущей версии, только заметно уже: единственный
//! способ узнать точнее — подключиться самим и последить, а это ручная
//! проверка (`CLAUDE.md`).
