...
//! единственный вызывающий `tunnel_state::any_tunnel_carries` в проде,
//! поставляющий ей реальные данные с этой машины.
//!
...
//!
//! `interface_alias` в `AdapterRoute` собирается здесь же (дёшево, тем же
//! проходом `GetAdaptersAddresses`, что и `is_tunnel_if_type`), но round 2
//! задачи 7 убрал его из решающих функций `tunnel_state` целиком — round 1
//! показал на реальной машине, что имя адаптера никогда не совпадает с
//! именем профиля OpenVPN, и держать его хоть где-то как признак «наш/чужой»
//! значило бы держать основание, уже доказанное неверным.
//!
...
/// маршрут принадлежит — прямой вход для `tunnel_state::any_tunnel_carries`.
///
...
/// вызовами — окно гонки физически возможно, но узкое), получает пустой
/// псевдоним и `is_tunnel = false`: `any_tunnel_carries` фильтрует именно по
/// `is_tunnel`, так что такая запись просто не попадёт в счёт — отказ в
/// консервативную сторону, а не паника и не догадка.
