...
mod tests {
    ...
    #[test]
    fn serde_masks_host_bits_on_load_same_as_from_str() {
        // Ревью fix round 1 (задача 5): три теста serde выше все брали уже
        // нормализованные строки, и ни один не проверял, что маскировка
        // хостовых битов (`FromStr`, см. выше) действительно происходит на
        // пути ЧЕРЕЗ `Deserialize`, а не только при прямом вызове `parse()`.
        // "203.0.113.5/24" из чужого конфига обязан прочитаться как
        // "203.0.113.0/24" — и именно в этой форме уйти обратно на диск при
        // следующем сохранении, иначе round-trip был бы нестабилен.
        let w: Wrapper = toml::from_str("net = \"203.0.113.5/24\"").expect("должен разобраться");
        assert_eq!(w.net.addr, Ipv4Addr::new(203, 0, 113, 0));
        assert_eq!(w.net.prefix, 24);
        let text = toml::to_string(&w).expect("должен сериализоваться");
        assert_eq!(text.trim(), "net = \"203.0.113.0/24\"");
    }
