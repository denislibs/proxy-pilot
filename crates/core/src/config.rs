...
pub struct Config {
    ...
    /// GUID сетей NLM — по ним опознаётся, где мы физически находимся («мы
    /// в офисе»); маршрут из GUID не выводится.
    ///
    /// **Не то же самое, что `office_subnets` ниже** — там CIDR-подсети
    /// самого офиса для маршрутов туннеля, данные другой природы с похожим
    /// именем.
    #[serde(default)]
