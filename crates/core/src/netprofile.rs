...
pub struct AdapterConfig {
    ...
    /// Текущая маска подсети, если адаптер на статике.
    ///
    /// Добавлена по тому же доводу, что и `dns`, и ради того же: чтобы
    /// «уже настроено верно» было утверждением, которое эта функция может
    /// проверить сама. Без маски вывод «всё совпадает» опирался бы на
    /// договорённость с задачей 6 («мы всегда ставим адрес и маску одним
    /// вызовом, значит маска не могла разойтись в одиночку») — довод
    /// верный, но живущий в комментарии и проверяемый только дисциплиной
    /// чужого кода. Поле превращает предположение в сравнение.
...
pub fn decide_profile(
    ...
    if in_office {
        ...
        // выгоды. Сравниваем всё, что ставим: адрес, маску и DNS. Неполное
        // сравнение здесь опаснее лишней записи: «уже верно» при разошедшейся
        // маске оставило бы машину сломанной молча и навсегда.
        // Заметка ревью round 2 (задача 6, Minor): `Vec<Ipv4Addr> ==
        // Vec<Ipv4Addr>` чувствительно к порядку. Если бы Windows однажды
        // вернула наши же DNS-серверы в другом порядке (адаптер читается
        // через `GetAdaptersAddresses`, задача 6), `already_correct` не
        // сработала бы, и `SetStatic` тихо повторялся бы на каждый цикл —
        // не порча данных (тот же адрес и тот же список серверов), а
        // лишняя запись `netsh` каждые ~30 секунд. Ни один из нас (задачи
        // 5 и 6) не может проверить на реальном адаптере, действительно ли
        // порядок стабилен между вызовами API — оставлено как известный,
        // но неподтверждённый край.
        let already_correct = !current.dhcp
            && current.set_by_us
            && current.addr == Some(ip)
            && current.mask == Some(mask)
...
mod tests {
    ...
    #[test]
    fn matching_address_and_dns_but_wrong_mask_is_still_reapplied() {
        // Маска — третья часть того, что мы ставим, и без её сравнения
        // «уже верно» было бы утверждением, которое эта функция проверить
        // не может: адрес совпал, DNS совпал, а машина при этом в чужой
        // подсети. Молчаливое `LeaveAlone` оставило бы её такой навсегда.
        let mut current = our_static_matching_profile();
        current.mask = Some(Ipv4Addr::new(255, 255, 0, 0));
        assert_eq!(
            decide_profile(true, &sample_profile(), &current),
            expected_static()
        );
