...
pub struct AppliedState {
    ...
    /// Псевдоним адаптера, на который это было поставлено — тот, что
    /// понимает `netsh ... name=`. Нужен отдельно от текущего опознания
    /// через NLM (`adapter::gather_from_nlm`), потому что тот путь работает,
    /// только пока адаптер ещё числится в офисной сети: как только машина
    /// покидает офис, NLM перестаёт отдавать это подключение вовсе, а
    /// откатывать статику в DHCP всё ещё нужно на том же самом физическом
    /// адаптере. Источник истины на этот момент — только собственная
    /// память службы.
...
mod tests {
    ...
    fn save_then_load_round_trips() {
        ...
        let state = AppliedState {
            ip: Some(addr(203, 0, 113, 10)),
            mask: Some(addr(255, 255, 255, 0)),
            iface: Some("OfficeAdapter".to_string()),
        };
